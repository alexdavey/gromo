...
class GrowingModule(torch.nn.Module):
    ...
    def prune(
        ...
        mask = selector_fn(self.weight, self.bias)

        assert isinstance(mask, torch.Tensor), "selector_fn mask must be a tensor."
        ...
        assert not mask.all(), f"Cannot prune all neurons from {self.name}."

        if not mask.any():
            return []

        self._prune_outputs(mask)

        if isinstance(self.next_module, GrowingModule):
            self.next_module._prune_inputs(mask)
        elif self.next_module is not None:
            raise NotImplementedError(
                f"Pruning {self.name} requires successor to be a GrowingModule."
            )

        pruned_indices = mask.nonzero(as_tuple=False).flatten().tolist()
        return pruned_indices

    def _prune_outputs(self, mask: torch.Tensor):
        """
        Remove output neurons based on mask.

        Parameters
        ----------
        mask : torch.Tensor
            Boolean tensor of shape ``(out_features,)`` where ``True`` identifies
            neurons to remove.
        """
        new_weight = self.weight[~mask].detach().clone()
        new_bias = self.bias[~mask].detach().clone() if self.use_bias else None

        # TODO: Add safe layer forward?
        self.layer = self.layer_of_tensor(new_weight, bias=new_bias)
        self._refresh_statistics_after_shape_change()

    def _prune_inputs(self, mask: torch.Tensor) -> None:
        """
        Drop the input channels that correspond to pruned outputs upstream.

        Parameters
        ----------
        mask : torch.Tensor
            Boolean tensor of shape ``(in_features_previous_layer,)`` generated by
            the previous module. ``True`` entries signal the channels that should be
            removed from this layer's weight matrix.
        """
        new_weight = self.weight[:, ~mask].detach().clone()

        # TODO: Add safe layer forward?
        self.layer = self.layer_of_tensor(new_weight, bias=self.bias)
        self._refresh_statistics_after_shape_change()
